...
export default function TaskPending({
  ...
  return (
    <div className={`p-6 w-full max-h-96 overflow-y-auto space-y-3 ${themeColor} rounded-2xl border border-amber-200 shadow-lg`}>
      {tasks.length === 0 ? (
        <div className="flex flex-col items-center justify-center h-32 text-amber-600">
          ...
          <p className="text-sm font-medium text-white">All tasks completed!</p>
        </div>
      ) : (
        tasks.map((task) => (
          <div
            key={task.id}
            className="bg-amber-50 p-4 rounded-lg shadow-sm border border-amber-200 hover:shadow-md transition-all duration-300"
          >
            <div className="flex items-center justify-between">
              <div className="flex items-center space-x-3">
                <div
                  className={`w-5 h-5 border-2  rounded-full cursor-pointer hover:bg-amber-50 transition-all duration-200 border-amber-500`}
